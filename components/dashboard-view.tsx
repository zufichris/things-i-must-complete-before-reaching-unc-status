...
import { CheckCircle, Clock, AlertCircle, BarChart2, PieChartIcon, LineChart } from "lucide-react"
...
export default function DashboardView({ projects, completedTasks, calculateProgress }: DashboardProps) {
  const [view, setView] = useState<"categories" | "projects" | "timeline">("categories")
  ...
  const categoryProgress = projects.reduce(
    (acc, project) => {
      ...
      const projectTasks = project.phases.reduce((sum: number, phase: any) => sum + phase.tasks.length, 0)
    ...
    {} as Record<
      string,
      { totalProjects: number; completedProjects: number; totalTasks: number; completedTasks: number }
    >,
  )

  // Prepare data for charts
  const categoryChartData = Object.entries(categoryProgress).map(([name, data]) => ({
    name,
    value: data.totalTasks > 0 ? Math.round((data.completedTasks / data.totalTasks) * 100) : 0,
    ...
    .flatMap((project) => {
      return project.phases.map((phase: any, index: number) => {
        ...
        const completedPhaseTasks =
          completedTasks[project.id]?.filter((taskId) => taskId.startsWith(`${index}-`)).length || 0
  ...
  const totalTasks = projects.reduce(
    (sum, project) => sum + project.phases.reduce((phaseSum: number, phase: any) => phaseSum + phase.tasks.length, 0),
    0,
  )

  const totalCompletedTasks = Object.values(completedTasks).reduce((sum, projectTasks) => sum + projectTasks.length, 0)
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="flex flex-col items-center">
                ...
                <div className="text-2xl font-bold">{projectStatuses.notStarted}</div>
                <div className="text-xs text-muted-foreground">Not Started</div>
              </div>
              <div className="flex flex-col items-center">
                <Clock className="h-5 w-5 text-warning mb-1" />
                <div className="text-2xl font-bold">{projectStatuses.inProgress}</div>
                <div className="text-xs text-muted-foreground">In Progress</div>
              </div>
              <div className="flex flex-col items-center">
                <CheckCircle className="h-5 w-5 text-success mb-1" />
                <div className="text-2xl font-bold">{projectStatuses.completed}</div>
          ...
          <CardContent>
            <div className="flex items-center">
              ...
              <div className="ml-4 text-sm text-muted-foreground">
                <div>Across {Object.keys(categoryProgress).length} categories</div>
                <div className="mt-1">
                  {Math.round((projectStatuses.completed / projects.length) * 100)}% completion rate
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent>
            <div className="flex items-center">
              ...
              <div className="ml-4 text-sm text-muted-foreground">
                ...
                <div className="mt-1">{Math.round((totalCompletedTasks / totalTasks) * 100)}% completion rate</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
                ...
                <div key={category.name} className="bg-card border rounded-lg p-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground whitespace-nowrap">
                      {category.completedProjects}/{category.totalProjects} projects
                    </span>
                  </div>
                </div>
              ))}
          ...
          <TabsContent value="timeline" className="mt-0">
            <div className="h-80">
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={timelineData} layout="vertical" margin={{ top: 5, right: 30, left: 100, bottom: 5 }}>
                    <XAxis type="number" />
                    <YAxis type="category" dataKey="name" width={100} />
                    <Tooltip />
                    <Bar dataKey="completed" stackId="a" fill="hsl(var(--chart-2))" />
                    <Bar dataKey="remaining" stackId="a" fill="hsl(var(--chart-3))" />
                  </BarChart>
                </ResponsiveContainer>
              </ChartContainer>
            </div>
