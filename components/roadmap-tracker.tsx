...
export default function RoadmapTracker() {
  ...
  const [activeView, setActiveView] = useState<
    "projects" | "dashboard" | "dependencies"
  >("projects");
  ...
  // Get current project
  const currentProject = roadmapData.find(
    (project) => project.id === activeProject,
  );

  // Get unique categories
  const categories = Array.from(
    new Set(roadmapData.map((project) => project.category)),
  );
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card rounded-lg border p-4 shadow-sm">
        <div className="flex items-center gap-4 w-full md:w-auto">
          <h2 className="text-xl font-bold">
            Projects I must Complete Before Reaching Unc Status
          </h2>
          <Tabs
            value={activeView}
              ...
              {currentProject ? (
                <Card className="card-hover">
                  <CardHeader>
                    ...
                    {currentProject.prerequisites &&
                      currentProject.prerequisites.length > 0 && (
                        <div className="mt-4">
                    ...
                    <div className="mt-6">
                      <div className="flex justify-between mb-2">
                        ...
                        <span className="text-sm font-medium">
                          {Math.round(calculateProgress(currentProject.id))}%
                        </span>
                      </div>
                      <div className="progress-bar-container">
                        <div
                          ...
                          style={{
                            width: `${calculateProgress(currentProject.id)}%`,
                          }}
                        />
                      </div>
                    </div>
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="space-y-1 mt-4">
                              {phase.tasks.map((task, taskIndex) => {
                                const taskId = `${phaseIndex}-${taskIndex}`;
                                const isCompleted = (
                                  completedTasks[currentProject.id] || []
                                ).includes(taskId);
                                const taskTime =
                                  taskTimes[currentProject.id]?.[taskId] || 0;
